...
import { X, ShoppingCart, MessageCircle, Package, CheckCircle, HelpCircle, Star, Share2, Check, ArrowLeft } from 'lucide-react';
...
export const ProductDetailModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-2.5 sm:p-4 md:p-6 animate-in fade-in duration-200">
      <div
        id="product-detail-modal"
        className="relative bg-white w-full max-w-3xl rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200 flex flex-col max-h-[94vh] sm:max-h-[90vh] overflow-hidden"
      >
        {/* Sticky Top Header Bar - Always visible regardless of scrolling on mobile/desktop */}
        <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-md px-4 sm:px-6 py-2.5 sm:py-3 border-b border-slate-200 flex items-center justify-between gap-3 shrink-0">
          ...
          <button
            ...
            className="p-1.5 sm:p-2 rounded-full bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-600 hover:text-slate-900 transition-colors shadow-xs shrink-0"
            title="Tutup Modal"
          >
        ...
        <div className="overflow-y-auto flex-1 overscroll-contain">
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="relative bg-slate-50 p-4 sm:p-6 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-200">
              {/* Product Image: object-contain with balanced height so the full image is visible without dominating mobile view */}
              <div className="relative w-full aspect-4/3 sm:aspect-square max-h-56 sm:max-h-72 md:max-h-80 max-w-sm rounded-2xl overflow-hidden shadow-xs bg-white flex items-center justify-center p-2 border border-slate-200/70">
                <img
                ...
                {hasDiscount && (
                  <div className="absolute top-2.5 right-2.5 bg-red-600 text-white text-[11px] sm:text-xs font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md shadow-md">
                    Diskon {discountPercent}%
                  </div>
                )}

                {product.angka_produk_favorit && (
                  <div className="absolute top-2.5 left-2.5 bg-amber-500 text-white text-[11px] sm:text-xs font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full flex items-center gap-1 shadow-md">
            ...
            <div className="p-5 sm:p-8 flex flex-col justify-between space-y-6">
              ...
              <div className="space-y-3 pt-4 border-t border-slate-200">
                ...
                <button
                  ...
                  onClick={() => setSelectedProductDetail(null)}
                  className="w-full py-2.5 px-4 rounded-xl border border-slate-300 hover:bg-slate-100 active:bg-slate-200 text-slate-700 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-colors"
                >
                  <ArrowLeft className="w-4 h-4 text-[#135A62]" />
